 ...
 *
 * Tools for managing footnotes and endnotes:
 * - word_add_footnote: Add a footnote at current position
 * - word_add_endnote: Add an endnote at current position
 * - word_get_footnotes: Get all footnotes
 * - word_get_endnotes: Get all endnotes
 * - word_delete_footnote: Delete a footnote
 * - word_delete_endnote: Delete an endnote
 */
...
async function executeWordGetFootnotes(_args: Record<string, unknown>): Promise<ToolResult> {
  try {
    ...
    if (response.success) {
      const footnotes = response['footnotes'] as Array<{
        index: number;
        text: string;
      ...
      const list = footnotes.map(fn =>
        `[${fn.index}] "${fn.text}" (ref: "${fn.reference_text?.slice(0, 30)}...")`
      ).join('\n');
...
async function executeWordGetEndnotes(_args: Record<string, unknown>): Promise<ToolResult> {
  try {
    ...
    if (response.success) {
      const endnotes = response['endnotes'] as Array<{
        index: number;
        text: string;
        reference_text: string;
      }> || [];
      ...
      const list = endnotes.map(en =>
        `[${en.index}] "${en.text}" (ref: "${en.reference_text?.slice(0, 30)}...")`
      ).join('\n');
