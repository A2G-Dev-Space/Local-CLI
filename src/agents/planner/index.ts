    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt > 1) {
          messages.push({
            role: 'user',
            content: `[RETRY ${attempt}/${maxRetries}] You MUST respond. Either:
...
DO NOT return an empty response. The user is waiting for your response.`,
          ...
          logger.warn(`Planning LLM retry attempt ${attempt}/${maxRetries}`);
        }
        ...
        if (toolCalls && toolCalls.length > 0) {
          ...
          try {
            ...
            lastError = new Error('Planning LLM returned invalid JSON for tool arguments.');
            continue; // Retry
          }
        ...
        logger.warn(`Planning LLM returned empty response (attempt ${attempt}/${maxRetries})`);
        lastError = new Error('Planning LLM returned empty response');
        // Continue to next retry
      } catch (error) {
        // Network or API error - will retry
        logger.warn(`Planning LLM error (attempt ${attempt}/${maxRetries}):`, error as Error);
