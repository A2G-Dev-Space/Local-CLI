...
import {
  ...
  emitTodoComplete,
  emitTodoFail,
  emitAutoSyncNotification,
  emitCompact,
  emitAssistantResponse,
} from '../tools/llm/simple/file-tools.js';
import { handleTodoCompleteAutoSync, buildCompactHistory } from '../core/background-sync.js';
...
// Web session: no Windows desktop path (Linux container)
...
/**
 * Module-level user system prompt for injection into buildSystemPrompt()
 * Set by session-server.ts when the API sends update_system_prompt
 */
let _userSystemPrompt: string = '';

export function setUserSystemPrompt(prompt: string): void {
  _userSystemPrompt = prompt;
}
...
export class PlanExecutor {
  ...
  async executePlanMode(
    ...
    try {
      ...
      logger.info('Tool definitions for LLM', { toolCount: tools.length, toolNames: tools.map(t => t.function.name) });
      ...
      const result = await llmClient.chatCompletionWithTools(messagesForLLM, tools, {
        ...
        onAfterToolExecution: async (loopMessages) => {
          // Web session: proactive auto-compact at 70% threshold inside tool loop
  ...
  async resumeTodoExecution(
    ...
    try {
      ...
      // Setup TODO callbacks (with background auto-sync)
      this.setupTodoCallbacks(currentTodos, callbacks, (updated) => {
        currentTodos = updated;
      }, {
        userMessage,
  ...
  private setupTodoCallbacks(
    ...
    setTodoWriteCallback(async (newTodos: TodoInput[]) => {
      ...
      if (allComplete && autoSyncContext) {
        ...
        const currentMsgs = autoSyncContext.getMessages();
        handleTodoCompleteAutoSync({
          todoTitle: completedTitles,
          todoId: updatedTodos[updatedTodos.length - 1]!.id,
          allTodos: updatedTodos,
          historyContext: buildCompactHistory(currentMsgs),
          userMessage: autoSyncContext.userMessage,
          llmClient: autoSyncContext.llmClient,
          notifyCallback: (result) => {
            emitAutoSyncNotification(result);
          },
        }).catch(() => {});
      }
