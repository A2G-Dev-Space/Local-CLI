...
/**
 * Module-level user system prompt for injection into planning prompt
 * Set by session-server.ts when the API sends update_system_prompt
 */
let _planningUserSystemPrompt: string = '';

export function setPlanningUserSystemPrompt(prompt: string): void {
  _planningUserSystemPrompt = prompt;
}
...
export class PlanningLLM {
  ...
  async generateTODOList(
    ...
    let systemPrompt = buildPlanningSystemPrompt(toolSummary, optionalToolsInfo, undefined, researchUrls);
    if (_planningUserSystemPrompt) {
      systemPrompt += `\n\n## Additional Instructions from User\n${_planningUserSystemPrompt}`;
    }
