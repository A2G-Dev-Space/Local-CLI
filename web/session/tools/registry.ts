 ...
 * Web session: browser and vision only (no office, no desktop-control)
 */
...
class ToolRegistry {
  ...
  async enableToolGroup(groupId: string, persist: boolean = true, skipValidation: boolean = false): Promise<EnableResult> {
    ...
    if (!group) {
      logger.errorSilent('Tool group not found', { groupId } as any);
    ...
    if (persist) {
      configManager.enableTool(groupId).catch((err) => {
        logger.errorSilent('Failed to persist tool enable state', err);
      });
    }
  ...
  async disableToolGroup(groupId: string, persist: boolean = true): Promise<boolean> {
    ...
    // Call onDisable callback (e.g., shutdown browser server)
    if (group.onDisable) {
      try {
    ...
    // Persist state to config
    if (persist) {
      configManager.disableTool(groupId).catch((err) => {
        logger.errorSilent('Failed to persist tool disable state', err);
      });
    }
...
export function initializeToolRegistry(): void {
  ...
  // LLM Simple Tools - System utilities
  toolRegistry.registerAll(SYSTEM_TOOLS);

  // LLM Simple Tools - Shell tools (always bash in web session)
