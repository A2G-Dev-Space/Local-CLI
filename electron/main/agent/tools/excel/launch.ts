 ...
 * Excel Launch Tools
 *
 * Basic Excel operations: launch, create, open, save, close, quit, screenshot
 * Total: 7 tools
 */
...
async function executeExcelClose(args: Record<string, unknown>): Promise<ToolResult> {
  try {
    ...
    const response = await excelClient.excelClose(save);
    if (response.success) {
      return { success: true, result: `Workbook closed${save ? ' (saved)' : ''}` };
    }
...
async function executeExcelQuit(args: Record<string, unknown>): Promise<ToolResult> {
  try {
    const save = args['save'] === true;
    const response = await excelClient.excelQuit(save);
    if (response.success) {
      return { success: true, result: `Excel closed${save ? ' (all workbooks saved)' : ''}` };
    }
...
const EXCEL_SCREENSHOT_DEFINITION: ToolDefinition = {
  ...
  function: {
    ...
    description: `Take a screenshot of the current Excel workbook.`,
    parameters: {
      type: 'object',
      properties: {
...
async function executeExcelScreenshot(_args: Record<string, unknown>): Promise<ToolResult> {
  try {
    ...
    if (response.success && response.image) {
      ...
      return { success: true, result: `Excel screenshot saved to: ${filePath}` };
    }
