 ...
 * - 테마, 레이아웃 등 설정 관리
 *
 * NOTE: This is Electron-specific config manager.
 * CLI config-manager handles LLM endpoints, while this handles app UI settings.
 * LLM endpoint management is in llm-manager.ts for Electron.
 *
 * CLI parity: src/core/config/config-manager.ts (structure only, content differs)
 */
