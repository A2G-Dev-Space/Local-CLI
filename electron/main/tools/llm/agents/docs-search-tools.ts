 ...
 * CLI parity: src/tools/llm/agents/docs-search-tools.ts
 *
...
import { getDocsPath } from '../../../core/docs-manager';
import type { ToolDefinition } from '../../../core';

// Reuse tell_to_user callback from user-interaction-tools
export { setTellToUserCallback } from '../simple/user-interaction-tools';

/**
 * Get base path for documentation
 * Uses the docsPath from docs-manager for consistency
 */
...
export const LIST_DIRECTORY_TOOL: ToolDefinition = {
  ...
  function: {
    name: 'list_directory',
    description: `List contents of a directory in the docs folder.
...
- "Finding API reference files"`,
        ...
        path: {
          type: 'string',
          description: 'Relative path from docs directory. Use "" or "/" for root.',
        },
      },
...
- "Checking example code for this feature"`,
        ...
        path: {
          type: 'string',
          description: 'Relative path to the file from docs directory',
        },
      },
      required: ['reason', 'path'],
    },
 ...
 * @param onTellUser Callback for tell_to_user messages
...
async function executeListDirectory(
  ...
  try {
    const DOCS_BASE_PATH = getDocsBasePath();
