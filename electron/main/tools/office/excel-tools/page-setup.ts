...
async function executeExcelSetPageSetup(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  logger.toolStart('excel_set_page_setup', args);
  try {
    const options: {
    ...
    if (response.success) {
      logger.toolSuccess('excel_set_page_setup', args, {}, Date.now() - startTime);
      return { success: true, result: response.message || 'Page setup configured' };
    }
    logger.toolError('excel_set_page_setup', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to set page setup' };
  } catch (error) {
    logger.toolError('excel_set_page_setup', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelSetPrintArea(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_set_print_area', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_set_print_area', args, { range: args['range'] }, Date.now() - startTime);
      return { success: true, result: response.message || `Print area set to '${args['range']}'` };
    }
    logger.toolError('excel_set_print_area', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to set print area' };
  } catch (error) {
    logger.toolError('excel_set_print_area', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelSetPrintTitles(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_set_print_titles', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_set_print_titles', args, {}, Date.now() - startTime);
      return { success: true, result: response.message || 'Print titles configured' };
    }
    logger.toolError('excel_set_print_titles', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to set print titles' };
  } catch (error) {
    logger.toolError('excel_set_print_titles', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
