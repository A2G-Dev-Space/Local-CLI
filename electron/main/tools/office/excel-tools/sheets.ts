...
async function executeExcelCopySheet(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_copy_sheet', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_copy_sheet', args, { newSheet: response['new_sheet_name'] }, Date.now() - startTime);
      return { success: true, result: `Sheet copied: ${response['new_sheet_name']}` };
    }
    logger.toolError('excel_copy_sheet', args, new Error(response.error || 'Failed to copy sheet'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to copy sheet' };
  } catch (error) {
    logger.toolError('excel_copy_sheet', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelSetTabColor(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_set_tab_color', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_set_tab_color', args, { sheet_name: args['sheet_name'] }, Date.now() - startTime);
      return { success: true, result: `Tab color set for "${args['sheet_name']}"` };
    }
    logger.toolError('excel_set_tab_color', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to set tab color' };
  } catch (error) {
    logger.toolError('excel_set_tab_color', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelSetZoom(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_set_zoom', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_set_zoom', args, { zoom_level: args['zoom_level'] }, Date.now() - startTime);
      return { success: true, result: `Zoom set to ${args['zoom_level']}%` };
    }
    logger.toolError('excel_set_zoom', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to set zoom' };
  } catch (error) {
    logger.toolError('excel_set_zoom', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelSetGridlines(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_set_gridlines', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_set_gridlines', args, { show: args['show'] }, Date.now() - startTime);
      return { success: true, result: `Gridlines ${args['show'] ? 'shown' : 'hidden'}` };
    }
    logger.toolError('excel_set_gridlines', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to set gridlines' };
  } catch (error) {
    logger.toolError('excel_set_gridlines', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelSetViewMode(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_set_view_mode', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_set_view_mode', args, { mode: args['mode'] }, Date.now() - startTime);
      return { success: true, result: `View mode set to "${args['mode']}"` };
    }
    logger.toolError('excel_set_view_mode', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to set view mode' };
  } catch (error) {
    logger.toolError('excel_set_view_mode', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelInsertPageBreak(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_insert_page_break', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_insert_page_break', args, { cell: args['cell'], type: args['type'] }, Date.now() - startTime);
      return { success: true, result: `Inserted ${args['type']} page break at ${args['cell']}` };
    }
    logger.toolError('excel_insert_page_break', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to insert page break' };
  } catch (error) {
    logger.toolError('excel_insert_page_break', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelDeletePageBreak(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_delete_page_break', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_delete_page_break', args, { cell: args['cell'], type: args['type'] }, Date.now() - startTime);
      return { success: true, result: `Deleted ${args['type']} page break at ${args['cell']}` };
    }
    logger.toolError('excel_delete_page_break', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to delete page break' };
  } catch (error) {
    logger.toolError('excel_delete_page_break', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelResetPageBreaks(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_reset_page_breaks', args);
  try {
    const response = await excelClient.excelResetAllPageBreaks(
      args['sheet'] as string | undefined
    );
    if (response.success) {
      logger.toolSuccess('excel_reset_page_breaks', args, {}, Date.now() - startTime);
      return { success: true, result: 'Reset all page breaks' };
    }
    logger.toolError('excel_reset_page_breaks', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to reset page breaks' };
  } catch (error) {
    logger.toolError('excel_reset_page_breaks', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
