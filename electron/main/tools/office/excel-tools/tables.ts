...
async function executeExcelCreateTable(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  logger.toolStart('excel_create_table', args);
  try {
    const range = typeof args['range'] === 'string' ? args['range'].trim() : '';
    const tableName = typeof args['table_name'] === 'string' ? args['table_name'].trim() : '';
    if (!range || !tableName) {
      return { success: false, error: 'Missing required arguments: range (e.g., "A1:G10") and table_name are required' };
    }
    ...
    if (response.success) {
      logger.toolSuccess('excel_create_table', args, { table_name: args['table_name'] }, Date.now() - startTime);
      return { success: true, result: response.message || `Table '${args['table_name']}' created` };
    }
    logger.toolError('excel_create_table', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to create table' };
  } catch (error) {
    logger.toolError('excel_create_table', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelDeleteTable(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_delete_table', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_delete_table', args, { table_name: args['table_name'] }, Date.now() - startTime);
      return { success: true, result: response.message || `Table '${args['table_name']}' deleted` };
    }
    logger.toolError('excel_delete_table', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to delete table' };
  } catch (error) {
    logger.toolError('excel_delete_table', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelGetTables(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_get_tables', args);
  try {
    const response = await excelClient.excelGetTables(args['sheet'] as string | undefined);
    if (response.success) {
      logger.toolSuccess('excel_get_tables', args, { count: (response as { count?: number }).count }, Date.now() - startTime);
      return { success: true, result: JSON.stringify(response, null, 2) };
    }
    logger.toolError('excel_get_tables', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to get tables' };
  } catch (error) {
    logger.toolError('excel_get_tables', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelAddTableColumn(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_add_table_column', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_add_table_column', args, { column_name: args['column_name'] }, Date.now() - startTime);
      return { success: true, result: response.message || `Column '${args['column_name']}' added` };
    }
    logger.toolError('excel_add_table_column', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to add table column' };
  } catch (error) {
    logger.toolError('excel_add_table_column', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
