...
async function executeExcelClearArrows(_args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_clear_arrows', _args);
  try {
    const response = await excelClient.excelClearArrows();
    if (response.success) {
      logger.toolSuccess('excel_clear_arrows', _args, {}, Date.now() - startTime);
      return { success: true, result: response.message || 'Cleared all arrows' };
    }
    logger.toolError('excel_clear_arrows', _args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to clear arrows' };
  } catch (error) {
    logger.toolError('excel_clear_arrows', _args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
