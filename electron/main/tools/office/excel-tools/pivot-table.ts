...
async function executeExcelCreatePivotTable(args: Record<string, unknown>): Promise<ToolResult> {
  ...
  logger.toolStart('excel_create_pivot_table', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_create_pivot_table', args, { tableName: response['table_name'] }, Date.now() - startTime);
      return { success: true, result: `Pivot table created: ${response['table_name']}` };
    }
    logger.toolError('excel_create_pivot_table', args, new Error(response.error || 'Failed to create pivot table'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to create pivot table' };
  } catch (error) {
    logger.toolError('excel_create_pivot_table', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelAddPivotField(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_add_pivot_field', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_add_pivot_field', args, { tableName: args['table_name'], fieldName: args['field_name'], orientation: args['orientation'] }, Date.now() - startTime);
      return { success: true, result: response.message || `Field '${args['field_name']}' added as ${args['orientation']}` };
    }
    logger.toolError('excel_add_pivot_field', args, new Error(response.error || 'Failed to add pivot field'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to add pivot field' };
  } catch (error) {
    logger.toolError('excel_add_pivot_field', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelRefreshPivotTable(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_refresh_pivot_table', args);
  try {
    const response = await excelClient.excelRefreshPivotTable(args['table_name'] as string);
    if (response.success) {
      logger.toolSuccess('excel_refresh_pivot_table', args, { tableName: args['table_name'] }, Date.now() - startTime);
      return { success: true, result: response.message || `Pivot table '${args['table_name']}' refreshed` };
    }
    logger.toolError('excel_refresh_pivot_table', args, new Error(response.error || 'Failed to refresh pivot table'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to refresh pivot table' };
  } catch (error) {
    logger.toolError('excel_refresh_pivot_table', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
...
async function executeExcelDeletePivotField(args: Record<string, unknown>): Promise<ToolResult> {
  const startTime = Date.now();
  logger.toolStart('excel_delete_pivot_field', args);
  try {
    ...
    if (response.success) {
      logger.toolSuccess('excel_delete_pivot_field', args, { tableName: args['table_name'], fieldName: args['field_name'] }, Date.now() - startTime);
      return { success: true, result: response.message || `Field '${args['field_name']}' removed from pivot table '${args['table_name']}'` };
    }
    logger.toolError('excel_delete_pivot_field', args, new Error(response.error || 'Failed'), Date.now() - startTime);
    return { success: false, error: response.error || 'Failed to delete pivot field' };
  } catch (error) {
    logger.toolError('excel_delete_pivot_field', args, error instanceof Error ? error : new Error(String(error)), Date.now() - startTime);
