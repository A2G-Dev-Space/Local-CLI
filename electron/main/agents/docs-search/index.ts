 ...
 *
 * CLI parity: src/agents/docs-search/index.ts
 */

import type { LLMClient } from '../../core/llm';
import type { Message, ToolDefinition } from '../../core';
import { BaseAgent, type AgentContext, type AgentResult, type AgentConfig } from '../base/base-agent';
import { DOCS_SEARCH_SYSTEM_PROMPT, buildDocsSearchUserMessage } from '../../prompts/agents/docs-search';
import { DOCS_SEARCH_TOOLS, createDocsToolExecutor } from '../../tools/llm/agents/docs-search-tools';
...
export class DocsSearchAgent extends BaseAgent {
  ...
  async execute(input: string, _context?: AgentContext): Promise<AgentResult> {
    ...
    try {
      ...
      logger.exit('DocsSearchAgent.execute', { success: true, iterations, duration });
