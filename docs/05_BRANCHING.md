...
| `postinstall` | `node scripts/patch-yoga.js` | `node scripts/patch-yoga.js` |
| `bun:build` | `bin/lcli` 빌드 | `bin/nexus` 빌드 |
| `open` 의존성 | 없음 | 있음 |
| `react-devtools-core` | 있음 (devDependencies) | 있음 (devDependencies) |
| `@yao-pkg/pkg` | 없음 | 있음 (devDependencies) |

**참고**: main 브랜치도 바이너리 배포를 지원하므로, cherry-pick 시 바이너리 관련 스크립트가 충돌할 수 있습니다. 항상 nexus-coder 버전의 경로(`bin/nexus`)를 유지하세요.

### ⚠️ browser-client.ts CDP 포트 주의사항

**nexus-coder 브랜치는 CDP 포트가 다릅니다!**

| 항목 | main (LOCAL-CLI) | nexus-coder (NEXUS) |
|------|------------------|---------------------|
| CDP 포트 | `9222` (Chrome 기본) | `8766` (사내망 호환) |

**배경:**
- 사내망에서 9222 포트(Chrome DevTools 기본 포트)가 보안 정책에 의해 차단됨
- WSL → Windows localhost 연결 시 9222 포트는 `empty reply from server` 오류 발생
- 8766 포트는 기존 browser-server.exe가 사용하던 포트로, 사내 방화벽에서 허용됨

**cherry-pick 시 주의:**
- main에서 browser-client.ts를 cherry-pick할 때 CDP 포트가 9222로 덮어씌워지지 않도록 주의
- 충돌 시 nexus-coder의 8766 포트 유지

```typescript
// nexus-coder 브랜치
private cdpPort: number = 8766;  // 사내망 호환

// main 브랜치
private cdpPort: number = 9222;  // Chrome 기본
```
...
```
1. git ls-remote로 최신 커밋 확인 (clone 없이)
2. ~/.nexus-coder/current-commit 파일과 비교
3. 버전이 다르면:
   a. git clone --depth 1 -b nexus-coder → ~/.nexus-coder/repo/
   b. bin/nexus.gz 압축 해제 → ~/.local/bin/nexus
   c. bin/yoga.wasm 복사 → ~/.local/bin/yoga.wasm
   d. ~/.nexus-coder/repo/ 삭제 (소스 코드 보호)
   e. ~/.nexus-coder/current-commit에 새 커밋 해시 저장
4. ~/.bashrc 또는 ~/.zshrc에 PATH 추가
5. npm unlink -g nexus-coder (기존 npm link 제거)
```

**소스 코드 보호**: 바이너리 추출 후 repo 디렉토리가 자동 삭제되어 소스 코드가 사용자 시스템에 남지 않습니다.
