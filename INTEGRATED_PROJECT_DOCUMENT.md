...
└─────────────────────────────────────────────────────────────┘
    ...
    │  (~/.a2g-cli/, Team configs, Presets, Profiles)    │
    └───────────────────────────────────────────────────┘
        │                                          │
    ┌───▼───────────────────────────────────────────▼────┐
    │     Local Document Repository & RAG System         │
    │  (~/.a2g-cli/docs/, Full-text search, Embeddings)  │
    └───────────────────────────────────────────────────┘
        │
    ┌───▼───────────────────────────────────────────────┐
...
a2g-cli/
├── bin/
│   ├── a2g (Shell wrapper - Unix/Linux)
│   ├── a2g.ps1 (PowerShell wrapper - Windows)
│   └── a2g.cmd (Batch wrapper - Windows)
├── runtime/
...
# 배포: git clone https://company.git/a2g-cli
# 실행: ./a2g-cli/bin/a2g 또는 ./a2g-cli/bin/a2g.cmd
```

#### 옵션 B: Standalone Binary
```
# Pkg.js를 사용하여 단일 실행 파일로 컴파일
a2g-cli-v1.0.0-linux-x64
a2g-cli-v1.0.0-darwin-x64
a2g-cli-v1.0.0-win-x64.exe
```
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ A2G-CLI v1.0.0 | 모델: gpt-4-turbo | 세션: 1h 23m | 메시지: 15     │  ← 헤더
├─────────────────────────────────────────────────────────────────────┤
│                                                                       │
│  [AI] 안녕하세요! A2G-CLI입니다. 무엇을 도와드릴까요?                │  ← 대화 영역
│                                                                       │
│  > 프로젝트 구조를 분석해줘                                          │
│                                                                       │
│  [AI] 프로젝트 구조를 분석하겠습니다.                                │
...
console = Console(theme=A2G_THEME)
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ A2G-CLI v1.0.0 | gpt-4-turbo | 00:15:23 | 8 msgs                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                       │
...
class A2GPrompt:
    ...
    def __init__(self):
        ...
        self.session = PromptSession(
            message='> ',
            history=FileHistory('~/.a2g-cli/history'),
...
# 사용 (환영 메시지 등)
typewriter_effect("안녕하세요! A2G-CLI입니다.")
```
...
✅ 대화 저장 완료
  ...
  • 메시지: 12개
  • 토큰 사용: 8,234개
  • 저장 위치: ~/.a2g-cli/sessions/optimization-task.json
...
# 메모리 새로고침 (모든 A2G-CLI.md 파일 재로드)
$ a2g /memory refresh

✅ 모든 프로젝트 컨텍스트 파일(A2G-CLI.md) 로드 완료
  • 발견된 파일: 5개
  • 새 메모리: 23개 항목 추가
```

#### 5.3 프로젝트 컨텍스트 파일 (A2G-CLI.md)
...
설정 관리:
  /settings          - 설정 편집기 열기 (~/.a2g-cli/settings.json)
  /theme             - 테마 변경 (dark/light/auto)
  /auth              - 인증 방법 변경
  /model             - 모델 선택 또는 목록 표시

파일/컨텍스트:
  /init              - A2G-CLI.md 파일 생성
...
명령어 정의 (~/.a2g-cli/commands/my-review.json):
{
  "name": "review",
...
│
├── 📁 bin/
│   ├── a2g (Unix/Linux shell wrapper)
│   ├── a2g.cmd (Windows batch wrapper)
│   └── a2g.ps1 (Windows PowerShell wrapper)
│
├── 📁 src/
│   ├── 📁 cli/
...
### API Key 관리
```typescript
// ~/.a2g-cli/endpoints.json의 API Key는 암호화되어야 함
{
...
### 1단계: 로컬 개발 환경 설정
```bash
git clone https://company.git/a2g-cli
cd a2g-cli
npm install
npm run build
npm test
...
```bash
# 테스트 서버에서
git clone file:///path/to/a2g-cli
cd a2g-cli
./bin/a2g --version
./bin/a2g --help
```
...
# 또는 Docker 이미지 빌드
docker build -t company/a2g-cli:v1.0.0 .
docker push company-registry/a2g-cli:v1.0.0
```
...
A2G-CLI는 2년 이상의 지속적인 개발을 통해 **엔터프라이즈급 오프라인 AI 코딩 어시스턴트**로 성장할 것입니다. 각 Phase에서 핵심 기능을 단계적으로 구축하고, 사용자 피드백을 반영하여 지속적으로 개선하는 것이 핵심입니다.
